...
import de.stefanteitge.mtmx.tools.internal.ModelViewerConfiguration;
...
public class ModelViewer {
	...
	public static void main(String[] args) {
		try {
			if (args.length != 3) {
				String msg = "You must provide game dir, POD file name and model path as arguments.";
				System.out.println(msg);
				System.exit(1);
			}
			(new ModelViewer()).run(args[0], args[1], args[2]);
	...
	public void run(ITriGameDir gameDir, PodFile podFile, String path) throws ViewerException {
		try {
			Engine engine = new Engine(gameDir, new ModelViewerConfiguration(podFile, path));
			engine.start("mtmX Model Viewer", false);
